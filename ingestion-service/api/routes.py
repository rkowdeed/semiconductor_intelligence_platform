...
def build_ingestion_router() -> APIRouter:
    ...
    for source_name, source in sources.items():
        if not source.enabled:
            continue
        _register_source_route(router, source_name, source.endpoint, source.method)

    return router


def _register_source_route(router: APIRouter, source_name: str, endpoint: str, method: str) -> None:
    async def handler(
        request: Request,
        ...
        result = ingestion_service.ingest(source_name, raw_body, session)
